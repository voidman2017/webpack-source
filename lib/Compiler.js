...
class Compiler {
	...
	constructor(context, options = /** @type {WebpackOptions} */ ({})) {
		/*note [hooks]:初始化了一系列的钩子
		使用的是 tapable
		tapable 是一个基于事件机制的插件机制，主要基于发布订阅模式实现
		*/
		...
		/*note []: 初始化了一些参数，包括 outputPath 、watching、options 等*/
	...
	run(callback) {
		 ...
		 * @returns {void}
		 */
		/*note [Compiler.run]: 定义一个onCompiled函数，主要用作传给 this.compile 作为执行的回调函数 */
		...
		/*note [Compiler.run]: compiler.run 主要流程：beforeRun -> run ,提供 plugin 执行时机 */
		const run = () => {
			/*note []: 注册在 beforeRun 阶段的 plugin 会在此时执行*/
			this.hooks.beforeRun.callAsync(this, err => {
				if (err) return finalCallback(err);
				/*note []: 注册在 run 阶段的 plugin 会在此时执行*/
...
${other}`);
	...
	compile(callback) {
		/*note []: 初始化 compilation 参数*/
		const params = this.newCompilationParams();
		/*note []: hooks.beforeCompile */
		this.hooks.beforeCompile.callAsync(params, err => {
			if (err) return callback(err);
			/*note []: hooks.compile */
			this.hooks.compile.call(params);
			/*note []: 返回一个 compilation ， newCompilation 中执行 hooks.thisCompilation -> hooks.compilation*/
			...
			this.hooks.make.callAsync(compilation, err => {
				...
				this.hooks.finishMake.callAsync(compilation, err => {
					...
					process.nextTick(() => {
						...
						/*note []:执行 compilation 的 finish 方法，对 module 上的错误或者警告处理*/
						compilation.finish(err => {
							logger.timeEnd("finish compilation");
							if (err) return callback(err);

							logger.time("seal compilation");
							/*note []: 执行 seal 对 module 代码进行封装输出*/
							compilation.seal(err => {
								logger.timeEnd("seal compilation");
								if (err) return callback(err);

								logger.time("afterCompile hook");
								/*note []: hooks.afterCompile */
