...
class Compilation {
	 ...
	 */
	/*note [compilation]: 1.初始化 constructor*/
...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	 ...
	 */
	/*note [compilation]: 2.通过 _addEntryItem 添加入口，编译需要从入口开始 */
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		/*note [compilation]: 调用addEntry钩子*/
		this.hooks.addEntry.call(entry, options);
		/*note [compilation]: 将当前的模块添加到 module tree（模块树）*/
		this.addModuleTree(
			{
				context,
				dependency: entry,
